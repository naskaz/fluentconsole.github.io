...
namespace ExcelMergeCellWriteComplexRowSkip
{
	internal class ExcelMergeCellWriteComplexRowSkip
	{
		static void Main(string[] args)
		{
			// Create a complex Excel file with merged cells with Row Skip
			using var writer = new ExcelWriter("ComplexReportRowSkip.xlsx", new ExcelOptions
			{
			...
			using (var sheet2 = writer.CreateWorksheet("Schedule"))
			{
				...
				sheet2.SkipRows(1);
				sheet2.MergeCells(9, 1, 9, 4); // move from row 8 to row 9
			...
			using (var sheet3 = writer.CreateWorksheet("Dashboard"))
			{
				...
				sheet3.SkipRows(1);                 // move from row 15 to row 16
			...
			Console.WriteLine("Merge Excel file with row skipping created successfully!");
		}
	}
}
