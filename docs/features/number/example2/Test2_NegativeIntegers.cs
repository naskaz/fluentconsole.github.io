...
namespace Test2_NegativeIntegers
{
    class Test2_NegativeIntegers
    {
        static void Main(string[] args)
        {
            Console.WriteLine("TEST 2: NEGATIVE INTEGERS");
            Console.WriteLine("==========================\n");

            var sorter = new HumanSort();
            var data = new List<string>
            {
                "-100", "-25", "-50", "-1", "-999", "-42", "-7", "-1000", "-500", "0"
            };
            ...
            DisplayList(asc);
            ValidateOrder(asc, true, "Negative integers ascending");

            Console.WriteLine("\nDescending Sort:");
            var desc = sorter.Sort(data, HumanSort.ColumnType.Number, false, HumanSort.NullHandling.NullsFirst, out _);
            DisplayList(desc);
            ValidateOrder(desc, false, "Negative integers descending");
