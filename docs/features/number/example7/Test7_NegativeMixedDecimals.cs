...
namespace Test7_NegativeMixedDecimals
{
    class Test7_NegativeMixedDecimals
    {
        static void Main(string[] args)
        {
            Console.WriteLine("TEST 7: NEGATIVE MIXED DECIMALS (varying decimal places)");
            Console.WriteLine("========================================================\n");
            ...
            DisplayList(asc);
            ValidateOrder(asc, true, "Negative mixed decimals ascending");

            Console.WriteLine("\nDescending Sort:");
            var desc = sorter.Sort(data, HumanSort.ColumnType.Number, false, HumanSort.NullHandling.NullsFirst, out _);
            DisplayList(desc);
            ValidateOrder(desc, false, "Negative mixed decimals descending");
